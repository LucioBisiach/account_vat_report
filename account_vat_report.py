    ...
    def generate_xlsx_report(self, workbook, data, vat_ledger):
        if vat_ledger.invoice_ids:
            ...
            if vat_ledger.type == 'sale':
                titles = ['Fecha','Cliente','CUIT','Tipo Comprobante','Responsabilidad AFIP','Nro Comprobante','Neto gravado','Neto Exento','IVA 21','IVA 10.5','IVA No gravado','Otros Impuestos','Total gravado']
            else:
                titles = ['Fecha','Cliente','CUIT','Tipo Comprobante','Responsabilidad AFIP','Nro Comprobante','Neto gravado','Neto Exento','IVA 21','IVA 10.5','IVA No gravado','Otros Impuestos','Percepción IVA 3%','Percepción IIBB','Total gravado']
            ...
            for i,obj in enumerate(vat_ledger.invoice_ids):
                ...
                if vat_ledger.type == 'sale':
                    ...
                    vat_taxable_amount = 0
                    vat_exempt_amount = 0
                    vat_amounts = [0,0,0,0,0]
                    for line in obj.invoice_line_ids:
                        if not line.tax_ids:
                            continue
                        tax = line.tax_ids[0]
                        if tax.amount == 0:
                            vat_exempt_amount = vat_exempt_amount + line.price_subtotal
                        elif tax.amount == 21:
                            vat_taxable_amount = vat_taxable_amount + line.price_subtotal
                            vat_amounts[0] = vat_amounts[0] + line.price_subtotal * (tax.amount / 100)
                            for imp in line.tax_ids:
                                if imp.amount == 3:
                                    vat_amounts[2] = vat_amounts[2] + line.price_subtotal * (imp.amount / 100)
                                if imp.tax_group_id.l10n_ar_tribute_afip_code == '07':
                                    vat_amounts[3] = vat_amounts[3] + line.price_subtotal * (imp.amount / 100)
                        elif tax.amount == 10.5:
                            vat_taxable_amount = vat_taxable_amount + line.price_subtotal
                            vat_amounts[1] = vat_amounts[1] + line.price_subtotal * (tax.amount / 100)
                            for imp in line.tax_ids:
                                # _logger.info("Impuestos %s", imp.tax_group_id.l10n_ar_tribute_afip_code)
                                if imp.amount == 3:
                                    vat_amounts[2] = vat_amounts[2] + line.price_subtotal * (imp.amount / 100)
                                if imp.tax_group_id.l10n_ar_tribute_afip_code == '07':
                                    vat_amounts[3] = vat_amounts[3] + line.price_subtotal * (imp.amount / 100)
                    sheet.write(row + index, 6, vat_taxable_amount,money_format)
                    sheet.write(row + index, 7, vat_exempt_amount,money_format)
                    sheet.write(row + index, 8, vat_amounts[0],money_format)
                    sheet.write(row + index, 9, vat_amounts[1],money_format)
                    sheet.write(row + index, 10, 0,money_format)
                    sheet.write(row + index, 11, 0,money_format)
                    sheet.write(row + index, 12, vat_amounts[2],money_format)
                    sheet.write(row + index, 13, vat_amounts[3],money_format)
